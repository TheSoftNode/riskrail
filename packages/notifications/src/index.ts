...
export function renderAlertEmail(input: AlertEmailInput): RenderedEmail {
  ...
  const subject = `RiskRail: ${input.metricLabel} ${input.comparison} ${input.threshold}`;

  const lines = [
    `${input.metricLabel} for ${short} is now ${input.observed}.`,
    '',
    `Rule: alert when ${input.metricLabel} ${input.comparison} ${input.threshold}`,
    `Source: ${input.source === 'on-chain' ? 'your on-chain risk policy' : 'a RiskRail alert rule'}`,
    '',
    `Review the position: ${input.dashboardUrl}`,
    '',
    'RiskRail is read-only and never moves funds. This is analytics, not',
    'financial advice.',
  ];
...
<div style="max-width:560px;margin:0 auto;padding:32px 24px">
  <p style="margin:0 0 24px;font-size:13px;letter-spacing:.12em;text-transform:uppercase;color:#94A3B8">RiskRail alert</p>
  ...
  <p style="margin:0 0 24px;font-size:13px;line-height:1.7;color:#94A3B8">
    Triggered by ${input.source === 'on-chain' ? 'your on-chain risk policy' : 'a RiskRail alert rule'}.
  </p>
  ...
  <p style="margin:28px 0 0;font-size:12px;line-height:1.7;color:#64748B">
    RiskRail is read-only and never moves funds. Metrics are estimates derived
